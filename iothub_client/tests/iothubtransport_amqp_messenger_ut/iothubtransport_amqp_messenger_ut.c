...
static void set_expected_calls_for_message_receiver_create()
{
	...
	// create_link_name()
	EXPECTED_CALL(malloc(IGNORED_NUM_ARG));
	STRICT_EXPECTED_CALL(UniqueId_Generate(IGNORED_PTR_ARG, 38)).IgnoreArgument_uid();
	...
	STRICT_EXPECTED_CALL(link_set_rcv_settle_mode(TEST_MESSAGE_RECEIVER_LINK_HANDLE, receiver_settle_mode_first));
