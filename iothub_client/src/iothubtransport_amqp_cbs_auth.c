...
static void mark_current_device_key_as_invalid(AUTHENTICATION_INSTANCE* instance)
{
...
int authentication_start(AUTHENTICATION_HANDLE authentication_handle, const CBS_HANDLE cbs_handle)
{
	...
	else
	{
		...
		if (instance->state != AUTHENTICATION_STATE_CLOSED)
		{
			result = __LINE__;
...
int authentication_stop(AUTHENTICATION_HANDLE authentication_handle)
{
	...
	else
	{
		...
		// Codes_SRS_IOTHUBTRANSPORT_AMQP_AUTH_09_032: [If `instance->state` is AUTHENTICATION_STATE_CLOSED, authentication_stop() shall fail and return __LINE__]
		if (instance->state == AUTHENTICATION_STATE_CLOSED)
		{
			result = __LINE__;
		...
		else
		{
			...
			update_state(instance, AUTHENTICATION_STATE_CLOSED);
				...
				if (verify_sas_token_refresh_timeout(instance, &is_timed_out) == RESULT_OK && is_timed_out)
				{
					...
					while ((device_key = get_current_valid_device_key(instance)) != NULL)
					{
						...
						else
						{
							...
							mark_current_device_key_as_invalid(instance);
						}
					}
		...
		else if (instance->state == AUTHENTICATION_STATE_STARTING)
		{
			...
			else
			{
				...
				while ((device_key = get_current_valid_device_key(instance)) != NULL)
				{
					...
					{
						LogError("Failed authenticating device '%s' using device keys (%d)", instance->device_id, instance->current_credential_in_use);
						mark_current_device_key_as_invalid(instance);
					}
				}
			}
